package com.codeazur.utils
{
	...
	{
		protected static var URL_REGEXP:RegExp = new RegExp("^(http|https|ftp)\://[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(:[a-zA-Z0-9]*)?/?([a-zA-Z0-9\-\._\?\,\'/\\\+&amp;%\$#\=~])*$");
		protected static var URL_REGEXP_SANS_SCHEME:RegExp = new RegExp("^[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(:[a-zA-Z0-9]*)?/?([a-zA-Z0-9\-\._\?\,\'/\\\+&amp;%\$#\=~])*$");
		...
		public static function cleanUri(input:String):String {
			input = trim(input);
			var a:Array;
			a = input.match(URL_REGEXP);
			if(a && a.length > 0) {
				return input;
			} else {
				a = input.match(URL_REGEXP_SANS_SCHEME);
				if(a && a.length > 0) {
					return "http://" + input;
				}
			}
			return "http://www.google.com/search?ie=UTF-8&q=" + escape(input);
		}
