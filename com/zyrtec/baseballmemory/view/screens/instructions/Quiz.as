package com.zyrtec.baseballmemory.view.screens.instructions
{
	...
	{
		private var _quizMC:QuizView; //holds the graphics
		...
		public function init():void {
			...
			for (var i:uint=0; i < _quizMC.answerArray.length; i++) {
				_questionArray[i].id = i;
				_questionArray[i].answer = _quizMC.answerArray[i];
				//trace("Quizzing = "+_questionArray[i].id, _quizMC.answerArray[i]);
			}
		...
		public function addEvents():void {
			...
			for each (var clip:MovieClip in _questionArray) {
				if (clip.id == _currentQuestion) {
					...
					clip.addEventListener(BaseballMemoryEvent.QUESTION_ANSWERED, onQuestionAnswered); //handle them pressing enter to submit
				}
			}
		}
		...
		public function removeEvents():void {
			...
			for each (var clip:MovieClip in _questionArray) {
				...
				clip.removeEventListener(BaseballMemoryEvent.QUESTION_ANSWERED, onQuestionAnswered);
			}
		}
		...
		private function showQuestion():void {
			...
			if (_currentQuestion<_questionTotal) {
				...
				this.dispatchEvent(new BaseballMemoryEvent(BaseballMemoryEvent.QUIZ_COMPLETE));
			}
		}
		...
		private function onShowError(e:BaseballMemoryEvent):void {
			_quizMC.errorMessage.visible=true;
		}
		
		private function onQuestionAnswered(e:BaseballMemoryEvent):void { 
			e.target.removeEventListener(BaseballMemoryEvent.QUESTION_ANSWERED, onQuestionAnswered);
			onSubmitClick();
		}
